...
const GamificationPage = () => {
  ...
  const fetchData = async () => {
    const [pointsRes, achRes, leaderRes] = await Promise.all([
      supabase.from("user_points").select("*").eq("user_id", user!.id).single(),
      supabase.from("user_achievements").select("*").eq("user_id", user!.id),
      supabase.from("user_points").select("*").order("total_points", { ascending: false }).limit(10),
    ]);
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
          <Trophy className="h-6 w-6 text-warning" /> Achievements & Progress
        </h1>
