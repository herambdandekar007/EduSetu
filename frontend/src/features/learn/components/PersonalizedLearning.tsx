...
interface PersonalizedLearningProps {
  ...
  onSelect?: (kind: keyof PersonalizedLearningRecommendation, value: string) => void;
}
  ...
  return (
    <section className="grid w-full grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {items.map(({ key, label, icon: Icon }) => {
        const rawVal = recommendation?.[key];
        const displayVal = rawVal && rawVal !== "-" ? rawVal : `Targeted ${label.toLowerCase()} for your syllabus`;

        return (
          <button
            key={key}
            type="button"
            onClick={() => onSelect?.(key, displayVal)}
            className="group flex flex-col justify-between rounded-xl border border-slate-200 bg-white p-4 text-left shadow-xs transition hover:border-indigo-300 hover:shadow-md cursor-pointer"
          >
            <div>
              <span className="mb-2 flex h-9 w-9 items-center justify-center rounded-lg bg-indigo-50 text-indigo-600 transition group-hover:bg-indigo-600 group-hover:text-white">
                <Icon size={16} />
              </span>
              <p className="text-xs font-semibold text-slate-500">{label}</p>
              <p className="mt-1 text-sm font-bold text-slate-900 leading-snug line-clamp-2">
                {displayVal}
              </p>
            </div>
